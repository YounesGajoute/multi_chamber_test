...
class IntegratedKeyboard(ttk.Frame):
    ...
    def _on_key_hover(self, button: ttk.Button, is_hovering: bool):
        """
        Toggle the built-in ttk 'active' state on hover.
        """
        if is_hovering:
    ...
    def _update_layout(self):
        """Update the keyboard layout based on shift/caps state."""
        self._create_keyboard()
...
class LoginTab:
    """
    Modern login interface tab with integrated keyboard and proper focus management.
    
    This class implements a login screen with username/password authentication
    and an integrated alphanumeric keyboard for easy touch input, with clear
    focus management and username retention after logout.
    """
    ...
    def _setup_styles(self):
        ...
        # Keyboard styles - all blue buttons
    ...
    def _create_modern_login_ui(self):
        """Create a professional card-based login interface with proper focus management."""
        ...
        self.username_field = EnhancedEntry(
            ...
            on_focus=self._on_username_focus,
            on_enter=self._on_username_enter
        )
        ...
        self.password_field = EnhancedEntry(
            ...
            is_password=True,
            on_focus=self._on_password_focus,
            on_enter=self._on_password_enter
        )
    ...
    def _set_initial_focus(self):
        """Set the initial focus to username field."""
        self.active_entry = self.username_field
        self.username_field.set_focus()
    
    def _on_username_focus(self, entry):
        """Handle username field receiving focus."""
        self.active_entry = self.username_field
        self.login_error.set("")  # Clear any error message
    
    def _on_password_focus(self, entry):
        """Handle password field receiving focus."""
    ...
    def _on_password_enter(self):
        """Handle Enter key in password field - attempt login."""
        self._handle_login()
    
    def _handle_keyboard_input(self, key):
        """
        Handle input from the integrated keyboard with proper focus management.
        
        Args:
            key: The key pressed on the keyboard
        """
        # Ensure we always have a focused entry
        ...
        if key == '\b':
            ...
            if current_entry == self.username_field:
                ...
                self._on_username_enter()
            else:
                # Attempt login from password field
    ...
    def _handle_login(self):
        """Process login authentication with proper focus management."""
        # Get input values
    ...
    def _handle_auth_result(self, username: str, role: Optional[str]):
        """
        Handle authentication result with proper state management.
        ...
        if role:
            ...
            # Set focus back to username field after failed login
            self.active_entry = self.username_field
            self.username_field.set_focus()
    
    def on_tab_selected(self):
        """Called when this tab is selected - clear all fields and set focus to username."""
        # Clear both fields for security
        self.username_var.set("")
        self.password_var.set("")
        self.login_error.set("")
        
        # Always start with username field focus
        self.active_entry = self.username_field
        self.username_field.set_focus()
    ...
    def logout(self):
        """Handle logout - clear all fields completely."""
        # Clear both fields for security
        self.username_var.set("")
        self.password_var.set("")
        self.login_error.set("")

...
class PrinterManager:
    ...
    def print_test_results(self, test_data: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            # Get reference from first chamber
...
^A0N,25,25^FO70,210^FDDATE:{date_str}^FS
^A0N,25,25^FO70,240^FDTime:{time_str}^FS
^A0N,50,50^FO70,140^FD{reference[3:] if reference and len(reference) > 3 and reference.startswith('G') else reference}^FS
^A0N,50,50^FO70,280^FDGROMMET TEST PASS^FS
^FT570,6
^BY1^BCR,50,Y,N,N^FDG{reference}^FS
^FT0,350
^BY1^BCB,50,Y,N,N^FDG{reference}^FS
^XZ"""
